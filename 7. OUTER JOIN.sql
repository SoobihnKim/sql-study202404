...
SELECT
  ...
  D.dept_nm
FROM tb_emp E
LEFT OUTER JOIN tb_dept D -- 왼쪽은 다 보여주고 오른쪽은 매칭되는것만 조회/OUTER 생략가능
ON E.dept_cd = D.dept_cd
;
...
-- LEFT JOIN의 결과: 46건 (INNER 41 + 부서없는사원 5)
-- RIGHT JOIN의 결과: 43건 (INNER 41 + 사원없는부서 2)
-- FULL JOIN의 결과: 48건 (INNER 41 + 5 + 2)
...
-- 상사 없는 사원도 조회하려면 LEFT JOIN으로 해야함
SELECT 
    A.emp_no "사원번호",
    A.emp_nm "사원명",
    A.addr "사원주소",
