...
--*
  ...
  D.dept_cd,
  D.dept_nm  
FROM tb_emp E, tb_dept D -- FROM에 나열하는 것은 오라클 문법
WHERE E.dept_cd = D.dept_cd
;

-- 사원의 사원번호와 사원명과 취득한 자격증명 조회
SELECT
    E.emp_no,
    E.emp_nm,
    E.sex_cd,
...
-- 사원의 사원번호와 사원명과 취득한 자격증명, 부서명 조회
SELECT
    E.emp_no,
    E.emp_nm,
    D.dept_nm,
...
-- 용인시에 사는 김씨인 사원의 사원번호, 사원명, 주소, 부서코드, 부서명을 조회하고 싶다.
SELECT
  E.emp_no,
  E.emp_nm,
  E.addr,
...
-- INNER JOIN 인에서는 해당 회원이 조회되지 않음
...
-- 조인 조건을 안 걸면 카테시안 곱이 만들어짐(모든 경우의 수)
SELECT
 *
FROM test_a, test_b;
...
--NATURAL JOIN은 공통 컬럼 알아서 찾기 때문에 앞에 식별자 안붙임
SELECT 
--*
...
-- 같은 이름 없으면 NATURAL JOIN 제대로 실행 안됨
SELECT
 *
FROM test_a
NATURAL JOIN test_b
